...
function SoulbindCacheOpener:updateBag(bagID)
...
-- Define rousings globally to avoid recreating it each time
local rousings = { [190315] = true, [190320] = true, [190322] = true, [190326] = true, [190328] = true, [190330] = true, [190451] = true };

function SoulbindCacheOpener:updateButtons()
    self:DebugLog("4 - updateButtons Called");
	updatingButtons = true;
    self.previous = 0;
    for i = 1, #self.items do
        self:DebugLog("5 - self.items loop");
        self:updateButton(self.items[i].button,self.items[i].id,self.items[i].threshold or 1,i);
    end
	updatingButtons = false;
end

function SoulbindCacheOpener:updateButton(btn,id,threshold,num)
    ...
    if (SoulbindCacheOpenerDB.rousing and meetThreshold) or (not rousings[id] and meetThreshold) then
        ...
        local point = SoulbindCacheOpenerDB.alignment;
        local relativePoint = point == "LEFT" and "RIGHT" or "LEFT";
        local xOffset = point == "LEFT" and 2 or -2;
...
function SoulbindCacheOpener:reset()
	self:DebugLog("8 - Reset Called")
	SoulbindCacheOpenerDB = {["enable"] = true,["alignment"] = "LEFT",["rousing"] = true};
	self.frame:SetPoint('CENTER', UIParent, 'CENTER', 0, 0);
	self:OnEvent("UPDATE");
end

function SoulbindCacheOpener:TryUpdateButtons()
	if updatingButtons and not shouldUpdateButtons then
		shouldUpdateButtons = true;
		C_Timer.After(0.1, function() SoulbindCacheOpener:TryUpdateButtons() end);
...
function SoulbindCacheOpener:AddButton()
	self:DebugLog("2 - Add Button Called");
	self.frame:Show();
	self:DebugLog("3 - Frame Shown");
...
function SoulbindCacheOpener:OnEvent(event, ...)
	...
	if UnitAffectingCombat("player") then
		self:DebugLog("10 - Player is in Combat");
		return
	end

	if event == "ADDON_LOADED" then
		self:DebugLog("0 - Addon Loaded");
	...
	if event == "PLAYER_LOGIN" then
		self:DebugLog("9 - Player Login Event");
		self.frame:UnregisterEvent("PLAYER_LOGIN");
	end 

	if event == "BAG_UPDATE" then
		SoulbindCacheOpener:updateBag(...);
	end

	self:DebugLog("1 - Event Called");
	SoulbindCacheOpener:TryUpdateButtons();
end
...
function SoulbindCacheOpener:DebugLog(message)
    if debug and DLAPI then 
        DLAPI.DebugLog("Testing", message) 
    end 
end
